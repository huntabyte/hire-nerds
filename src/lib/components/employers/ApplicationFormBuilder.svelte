<script lang="ts">
	...
	const handleSubmitQuestions = async () => {
		if (questions.length > 0 && questions[0].question.title !== '') {
		...
		goto(`/employers/jobs/${$page.data.job.id}`)
	}
</script>
...
<div class="w-full flex flex-col gap-4 p-4">
	{#if questions.length > 0}
		{#each questions as question}
			<div class="w-full flex flex-col gap-3 border border-surface-600/75 p-4 relative rounded-lg">
				{#if question.type === ''}
					<span in:slide|local={inTransitionParams} out:slide|local={outTransitionParams}>
						<h5 class="leading-none pb-2">Select question type</h5>
						<button
							type="button"
							class="btn btn-ghost-primary "
							on:click={() => (question.type = questionType.MULTIPLE_CHOICE)}
	...
	<div class="flex w-full gap-2 pt-6">
		<button type="button" class="btn btn-filled-primary w-full" on:click={() => handleAddQuestion()}
			>{questions.length > 0 ? 'Add Another Question' : 'Add Question'}</button
		>
